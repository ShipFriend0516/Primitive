...
const RecruitPage = () => {
  ...
  useEffect(() => {
    ...
    setTimeString(`${start.getFullYear()}년 모집 기간: ${start.getMonth() + 1}월 ${start.getDate()}일 (${weeks[start.getDay()]}) ~ ${end.getMonth() + 1}월 ${end.getDate()}일 (${weeks[end.getDay()]})`);
  ...
  const recruitData = {
    form: '/24/2024학년도 PRIMITIVE 신청서.hwp',
    ot: '/25/2025 PRIMITIVE 신입생 OT 자료.pptx',
    recruitNotice: '/25/25PrimitiveRecruit.webp',
    presidentData: {
      position: '회장',
      name: '곽민정',
      kakaoID: '2002kwak',
      email: 'kwak2002mj@gmail.com'
    },
    vicePresidentData: {
      position: '부회장',
      name: '이찬규',
      kakaoID: 'LCGPull',
      email: 'lay5137@naver.com'
    }
  }



  const disabledStyle = 'disabled:bg-gray-200 disabled:hover:shadow-xl disabled:cursor-not-allowed';
      ...
      <div className="flex-grow bg-slate-50 w-screen flex flex-col items-center justify-center py-32 gap-1">
        ...
        <div className="flex flex-col items-center mt-20">
          ...
          <div className="flex sm:flex-row flex-col text-white rounded-lg overflow-hidden text-nowrap">
            {[recruitData.presidentData, recruitData.vicePresidentData].map(el=>
