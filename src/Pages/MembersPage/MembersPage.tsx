        ...
        <div className="max-w-7xl mx-auto w-screen pb-24 flex flex-col items-center ">
          ...
          <div className="flex  gap-4 md:flex-row flex-col mb-24">
            <MemberCard
              ...
              description={`안녕하세요 PRIMITIVE 22대 회장 ${data.presidentData.name}니다.`}
            />
            <MemberCard
          ...
          <h2 className="text-3xl font-bold my-3 inline-flex items-center gap-1"><FaDev/>개발</h2>
          <div className="flex md:flex-row  flex-col gap-2">
            {developers.map((developer, index) => (
            <MemberCard name={developer.name} number={developer.grade} position={developer.position} description={developer.description} image={developer.image} type={"small"} key={index} />
            ))}
          </div>
        </div>
      </>
